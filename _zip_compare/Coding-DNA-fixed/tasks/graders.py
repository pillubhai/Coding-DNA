...
def _grade_easy(action: str, signals: dict) -> float:
    ...
    if action == "contain":
        return round(min(0.62 + 0.2 * (1.0 - fire_pressure) + 0.1 * (1.0 - structure_risk), 0.99), 3)
    if action == "suppress":
        return round(max(0.01, 0.4 + 0.1 * (1.0 - fire_pressure)), 3)
    return 0.12
...
def _grade_medium(action: str, signals: dict) -> float:
    ...
    structure_risk = signals.get("structure_risk", 0.5)
    if action == "suppress":
        return round(min(0.65 + 0.18 * fire_pressure + 0.07 * structure_risk, 0.99), 3)
    if action == "protect":
        return round(max(0.01, 0.45 + 0.12 * structure_risk), 3)
    return 0.15
...
def _grade_hard(action: str, signals: dict) -> float:
    ...
    structure_risk = signals.get("structure_risk", 0.5)
    if action == "protect":
        return round(min(0.7 + 0.15 * structure_risk, 0.99), 3)
    if action == "suppress":
        return round(max(0.01, 0.5 + 0.1 * fire_pressure), 3)
    return 0.12
...
def _grade_extreme(action: str, signals: dict) -> float:
    ...
    structure_risk = signals.get("structure_risk", 0.5)
    if action == "protect":
        return round(min(0.72 + 0.12 * fire_pressure + 0.08 * structure_risk, 0.99), 3)
    if action == "suppress":
        return round(max(0.01, 0.48 + 0.1 * fire_pressure), 3)
    return 0.14
